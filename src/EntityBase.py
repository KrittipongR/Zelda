...
class EntityBase():
    ...
    def Collides(self, target):
        return not(self.rect.x + self.width < target.x or self.rect.x > target.x + target.width or
                   self.rect.y + self.height < target.y or self.rect.y > target.y + target.height)
